...
async function fetchRequests() {
    try {
        ...
        requests = (data.requests || []).filter(r => r.category_id == 5 || r.category_name === "General");
...
async function loadTechniciansToDetailModal(selectedName = "") {
    ...
    data = data.filter(tech => tech.category_id == 5 || tech.category_name === "General");
...
async function loadAvailableTechniciansToDetailModal(selectedName = "", categoryId = null, date = null, time = null) {
    ...
    select.onchange = function () {
        const opt = select.selectedOptions[0];
        document.getElementById('detailContactInput').value = opt && opt.value !== "" ? (opt.dataset.phone || "") : "";
    };
    // ตั้งค่าเบอร์โทรทันทีถ้ามีชื่อเดิม
    if (selectedName) {
        const selected = Array.from(select.options).find(opt => opt.value === selectedName);
...
function formatTimeDisplay(dtString) {
    if (!dtString) return '';
    // ถ้าเป็น HH:MM
    if (/^\d{2}:\d{2}$/.test(dtString)) return dtString;
    // ถ้าเป็น ISO
    ...
    // ถ้าเป็น 11:30:00
    if (/^\d{2}:\d{2}:\d{2}$/.test(dtString)) {
        return dtString.slice(0, 5);
    }
    return dtString;
}

function formatTimeForInput(dtString) {
